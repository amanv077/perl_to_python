...
sub _thread_parent()
{
  ...
  ##
  ## Set mail address for MAIL FROM
  my $newmail = &genMail();
  if($_emails[0]  ne ""){
      $newmail = $_emails[0];
  }else{
...
#    if(openMail($_smtpserver{$_TDOMAIN}{$curserver}, $iplist[$ownip], $newmail.$_LOCAL{MAIL})){
    if(openMail($_smtpserver{$_TDOMAIN}{$curserver}, $iplist[$ownip], $newmail)){
...
sub openMail()
{
  ...
  print("Opening $_smtpserver : $port : from $_localip with $localserver FROM $_mailfrom\n");
  ##
  ## Socket による接続
  ##
