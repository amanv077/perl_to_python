...
##
$_files{ok} = $_files{ng} = $_files{ps} = $_files{ck} = $filepath.$filename;
if($ARGV[2] ne ""){
  print("Individual Saving $filename\n");
  $_dfiles{ok} = $_files{ng} = $_files{ps} = $_files{ck} = $filepath.$filename;
  $_dfiles{ok} =~ s/\.txt/\.ok\./;
